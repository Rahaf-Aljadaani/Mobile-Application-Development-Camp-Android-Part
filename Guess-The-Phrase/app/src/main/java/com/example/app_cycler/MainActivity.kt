...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        /*    sharedPreferences = this.getSharedPreferences(
            ...
            val myMessage = sharedPreferences.getString("myMessage", "").toString()  // --> retrieves data from Shared Preferences
                ...
                text.length == 1 -> {
                    ...
                    for (i in chars) {
                        if (i.toLowerCase() == text[0].toLowerCase()) {
                            GusseTextStar = GusseTextStar.substring(0, j) + text[0] + GusseTextStar.substring(j + 1)
                            find = true
